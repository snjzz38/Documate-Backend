        ...
        if (action === 'execute_step') {
            ...
            switch (step.tool.toUpperCase()) {
                ...
                case 'WRITE': {
                    ...
                    // Skip QA here if a downstream step (CITE or QUOTES) will re-check the text
                    const downstreamWillCheck = options.enableCite || options.enableQuotes;
                    if (!downstreamWillCheck) {
                ...
                case 'CITE': {
                    ...
                    // Limit input to 25 sentences to reduce token cost
                    const inputSentences = input.match(/[^.!?]+[.!?]+/g) || [input];
                    const trimmedInput = inputSentences.slice(0, 25).join(' ').trim();
                    const prompt = AgentPrompts.buildCitePrompt(trimmedInput, sourceList, citationFormat, type, hasStructuredHeadersCite);
                    ...
                    // Skip QA if QUOTES step follows — it will run checkWithGroq on the final text
                ...
                case 'QUOTES': {
                    ...
                    // Skip if the task doesn't explicitly ask for quotes or evidence
                    if (!/quote|evidence|support|cite directly|direct.*citation/i.test(context.task || '')) {
                        result.output = input;
