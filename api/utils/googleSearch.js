...
// ... (PipelineService remains exactly the same as previous working version) ...
const PipelineService = {
    ...
    generateFallback(source) {
        ...
        const isSiteName = author && (author === site || author.toLowerCase().includes(site.toLowerCase().replace(/\.(com|org|edu|net)/, '')));
    ...
    processInsertions(context, insertions, sources, formattedMap, outputType, style) {
        ...
        validInsertions.forEach(item => {
            ...
            if (isInvalid) citString = this.generateFallback(source);
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // --- NEW: AI SMART SEARCH ---
        // Generate a better query based on the user's text
        console.log("[Citation] Generating Smart Query...");
        const queryPrompt = CitationPrompts.buildSearchQuery(context);
        const smartQuery = await GroqAPI.chat([{ role: "user", content: queryPrompt }], GROQ_KEY, false);
        
        // Clean up the query (remove quotes or extra text if the model chatted)
        const cleanQuery = smartQuery.replace(/['"]/g, '').trim();
        console.log(`[Citation] Smart Query: "${cleanQuery}"`);

        // 2. SEARCH & SCRAPE
        const rawSources = await GoogleSearchAPI.search(cleanQuery, SEARCH_KEY, SEARCH_CX);
        ...
        if (outputType === 'bibliography') {
            prompt = CitationPrompts.buildBibliography(style, richSources, new Date().toLocaleDateString());
        } else {
            prompt = CitationPrompts.buildInsertion(style, context, richSources, new Date().toLocaleDateString());
        }
